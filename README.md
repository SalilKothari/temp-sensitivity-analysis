...
These values are extremely small, looking back, they were improperly scaled by temperature in Celsius, which is wrong for normalization. However the values show a decent picture for which parameters change the most due to temperature. From this basic analysis - Rds (switch and diode), Id_sat, Vt_thermal, and gds were affected by temperature. There were many NaNs present in the data due to one main reason, there not being enough data over a range of temperature values. Many measurements in the transistor database were only performed at 25 C, so analysis was not possible. Many libraries that were used only performed proper computations if there were at least 3 temperatures measured, which there weren't, leading to more NaNs. In the future, a more complete dataset should be used, with explicit measurements of values at different temps.
